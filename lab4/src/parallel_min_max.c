...
int main(int argc, char **argv) 
{
    ...
    printf("\n");*/  
	childPids = (int*)malloc((pnum + 1) * sizeof(int));
	childPids[pnum] = -2;
    if(!with_files)  
    {    
	    pipe(pipefd);  
    }  
    signal(SIGALRM, killProcesses);
    alarm(timeout);
	printf("Начало отсчета таймера\n");
...
void killProcesses()
{
	...
	printf("Всем процессам-потомкам отправлен сигнал SIGKILL\n");
}